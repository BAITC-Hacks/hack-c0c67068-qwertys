...
def load_weather(issue_time, horizon_hours, weather_dir=None):
    issue = utc(issue_time)
...
def predict_from_inputs(issue_time, turbine_ids, horizon_hours, weather, model_dir=None):
    ...
    return {"rows": output, "mode": "deterministic", "metadata": {"model_version": manifest["model_version"], "input_version": input_version, "weather_provider": first["provider"], "weather_model": first["model"], "weather_run_time": first["run_time"], "weather_available_at": first["available_at"], "availability_basis": first["availability_basis"], "scada_timezone": manifest["scada_timezone"], "timezone_status": "inferred", "provenance_status": first["provenance_status"]}, "warnings": ["Историческая погода из кэша; публикация на момент выпуска не подтверждена (+9h — допущение).", "Часовой пояс SCADA принят как фиксированный UTC+6; подтверждения владельца нет.", "Мощность в исходных нормализованных единицах; это не МВт и не энергия.", "Февральских фактических меток нет; февральские метрики не вычислялись."]}
