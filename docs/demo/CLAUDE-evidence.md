...
## 3-minute UI walkthrough for the jury (only features verified above)
...
| 2:20–2:50 | Панель «Качество на истории», вкладки v1/v2 | «Качество только на истории: январь 2026 не использовался для выбора; кривая мощности по NWP против CatBoost и persistence. v2 — помеченный post-test эксперимент. Метрик за февраль нет — факта нет.» |
| 2:50–3:00 | «Скачать CSV» | «Экспорт идентичен API — это проверил независимый участник команды.» |

If the API is down during the demo, do NOT use «Синтетический пример» as a result — it is labelled SYNTHETIC and only shows the data shape.
