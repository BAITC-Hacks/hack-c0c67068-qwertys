...
Metadata fields: model_version, input_version, weather_provider, weather_model, weather_run_time, weather_available_at, availability_basis, scada_timezone, timezone_status, provenance_status. All optional unknown values remain null. Use `inferred_run_plus_6h`/`unconfirmed` where evidence is inferred, not observed. HTTP validation rejects future weather, wrong request identity, duplicate hours, wrong leads, nonfinite or wholly missing numeric output. Partial coverage remains visible as warnings, without filling zeros.
