...
**Предфинальная приёмка 23.09.2026:** версия `746a0a8` прошла [независимый повторный аудит](coordination/research/C3/strict-reaudit-746a0a8.md): **94 теста и 15 подтестов, 30/30 адресных проверок, 2 Node-теста, сборка и реальный браузерный сценарий 24/48 ч → обновление → CSV → 29/29 replay**. Все семь исходных программных нарушений исправлены; выбор часа ограничен поддерживаемым 12Z. На контрольных 96 значениях численный прогноз сохранился точно. Эта версия опубликована в `main` как рабочий резерв.

Ранее [чистый Git-экспорт `1895090`](coordination/research/C2/clean-core-1895090.md) отдельно прошёл новую установку 46 зафиксированных Python-пакетов, загрузку погоды, CLI и API; последующие исправления проверены повторным аудитом выше. Историческая публикация погоды и часовой пояс SCADA остаются предположениями. Без готовой модели/кэша API возвращает `503 not_ready`. Синтетический пример UI явно помечен и не является прогнозом. [Сценарий демонстрации и описание решения](coordination/FINAL_DELIVERY.md).

## Быстрый запуск

![SAMAL: прогноз после обновления погоды](docs/demo/CLAUDE-FE-dashboard-desktop.png)

Реальный интерфейс и данные API, проверка Claude на ноутбуке 2. [Узкий экран](docs/demo/CLAUDE-FE-dashboard-narrow-500.png). Кадры сняты до финального уточнения текста происхождения погоды; значения прогнозов не менялись. Главная страница — 3D-иллюстрация; рабочий экран открывается кнопкой Dashboard или по `/#/dashboard`.

Требуются Python 3.12, Node.js 24, npm, Git. Команды выполняются из корня репозитория. Проверенная предфинальная версия опубликована в `main`; `coord/hackalem-start` используется для приёмки новых изменений перед переносом в main.
...
Полный набор после исправлений строгого аудита с установленными исследовательскими зависимостями: **94 теста и 15 подтестов PASS**, независимо повторено координатором и аудитором. Базовый lock не устанавливает Torch: соответствующие optional-тесты могут пропускаться. Синтетические стресс-проверки доказывают обработку сценариев, но не точность прогнозирования. [Снимки реального интерфейса и воспроизводимый сценарий](docs/demo/CLAUDE-evidence.md).

GitHub Actions настроен на Python-тесты, Node-проверку origin, сборку и lint без ключей и внешних датасетов. Локально эти команды прошли; результат hosted CI пока не подтверждён. Остаются 6 предупреждений lint и предупреждение о размере JS-бандла, без ошибок сборки. [Реестр исправлений и открытых ограничений](coordination/STRICT_AUDIT_REMEDIATION.md).
